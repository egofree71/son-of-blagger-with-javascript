 ...
 *
 * These sequences are not part of the 120 Hz gameplay simulation. They keep the
 * old 60 FPS reference timing, but advance from elapsed milliseconds so level
 * reveals, level transitions and the ending screen stay stable on every display.
 */
export const SEQUENCE_REFERENCE_FPS = 60;
export const SEQUENCE_LOGICAL_STEP_MS = 1000 / SEQUENCE_REFERENCE_FPS;

export const LEVEL_REVEAL_STEP_INTERVAL_STEPS = 2;
export const LEVEL_REVEAL_STEP_INTERVAL_MS = LEVEL_REVEAL_STEP_INTERVAL_STEPS * SEQUENCE_LOGICAL_STEP_MS;
